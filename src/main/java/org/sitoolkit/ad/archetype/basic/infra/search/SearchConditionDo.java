package org.sitoolkit.ad.archetype.basic.infra.search;
...
public class SearchConditionDo implements Serializable {
	...
	/**
	 * 画面入力された検索条件のマップ
	 * キー：演算子 + "_"  + 検索フィールド、値：検索に使用する値
	 */
	private Map<String, Object> params = new HashMap<String, Object>();

	/**
	 * クエリのパラメータとなるマップ
	 * キー：クエリのパラメータのプレースホルダ、値：プレースホルダに対応する値
	 */
	...
	/**
	 * searchCondition、pageCtrl、sortListを基に
	 * searchQuery、countQueryの文字列を構築する。
	...
	/**
	 * ソートキーのリストの先頭にソートキーを追加します。
	 * 追加するキーがリスト内の最初のキーと同じ場合、
	 * キーの追加は行わずリスト内の最初のキーの昇順・降順を反転します。
	 * 追加するキーが既にリスト内に存在する場合、
	 * 既存のキーをリストから除きます。
	 * リストのサイズがソート履歴件数以上である場合、
	 * リスト内の
	 *
	 * @param field 追加するソートキー
	 */
